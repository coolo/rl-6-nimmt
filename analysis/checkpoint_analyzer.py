...
class CheckpointAnalyzer:
    ...
    def plot_elo_by_cycle(self, save_path: str = None):
        ...
        plt.fill_between(
            cycle_stats["cycle"], cycle_stats["mean"] - cycle_stats["std"], cycle_stats["mean"] + cycle_stats["std"], alpha=0.3, label="±1 Std Dev"
        )
    ...
    def plot_score_vs_elo(self, save_path: str = None):
        ...
        scatter = plt.scatter(final_data["avg_score"], final_data["elo_rating"], c=final_data["games_played"], cmap="viridis", alpha=0.7, s=60)
        ...
        plt.text(0.05, 0.95, f"Correlation: {correlation:.3f}", transform=plt.gca().transAxes, bbox=dict(boxstyle="round,pad=0.3", facecolor="white"))
    ...
    def plot_elo_distribution(self, save_path: str = None):
        ...
        plt.axvline(final_elos.mean(), color="red", linestyle="--", label=f"Mean: {final_elos.mean():.1f}")
        plt.axvline(final_elos.median(), color="orange", linestyle="--", label=f"Median: {final_elos.median():.1f}")
    ...
    def plot_win_rates(self, save_path: str = None):
        ...
        win_proxy = self.elo_data.groupby("player_id").agg({"elo_rating": ["last", "count"], "elo_change": "mean"}).round(2)
        ...
        scatter = plt.scatter(win_proxy["final_elo"], win_proxy["avg_elo_change"], s=win_proxy["games_played"] * 2, alpha=0.6)
    ...
    def plot_performance_metrics(self, save_path: str = None):
        ...
        axes[1, 1].axvline(elo_improvements.mean(), color="red", linestyle="--", label=f"Mean: {elo_improvements.mean():.1f}")
    ...
    def generate_summary_report(self) -> Dict:
        ...
        report = {"timestamp": datetime.now().isoformat(), "data_summary": {}, "performance_metrics": {}, "training_progress": {}}
    ...
    def _save_raw_data(self, report: Dict, filename: str):
        ...
        raw_data = {
            ...
            "checkpoint_summary": {"total_checkpoints": len(self.checkpoint_data), "checkpoint_cycles": list(self.checkpoint_data.keys())},
        }
