...
class AdaptiveTraining:
    ...
    def run_adaptive_cycle(self, num_matches: int = 50, target_penalty: int = 100, train_after_matches: bool = True, log_frequency: int = 10):
        ...
        results = self.tournament.run_random_matches(num_matches, target_penalty=target_penalty, verbose=False)
    ...
    def save_models(self, directory: str):
        ...
        stats = {
            "player_stats": [
                {"id": p.player_id, "elo": p.elo_rating, "games": p.games_played, "total_score": p.total_score, "epsilon": p.epsilon} for p in self.players
            ]
        }
    ...
    def create_checkpoint(self, save_top_n: int = 10, save_all: bool = False, tournament_results: Optional[Dict] = None) -> str:
        ...
        checkpoint_path = self.checkpoint_manager.create_checkpoint(
            players=self.players, cycle=self.current_cycle, save_top_n=save_top_n, save_all=save_all, tournament_results=tournament_results
        )
    ...
    def get_progression_summary(self) -> Dict:
        ...
        if self.enable_checkpoints:
            ...
            summary.update({"total_checkpoints": len(checkpoints), "best_checkpoint": best_checkpoint})
    ...
    def finalize_training(self) -> Dict:
        ...
        for i, player in enumerate(top_players, 1):
            print(
                f"  {i:2d}. {player.player_id}: ELO {player.elo_rating:.1f} "
...
class SelfPlayTrainer:
    ...
    def __init__(self, model: Take6Network, learning_rate: float = 0.001, memory_size: int = 10000, batch_size: int = 32):
        self.model = model
