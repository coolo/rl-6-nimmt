...
class EloProgressionLogger:
    ...
    def __init__(self, log_dir: str = "logs", session_name: Optional[str] = None):
        ...
        self.detailed_log = {
            "session_info": {"session_name": session_name, "start_time": datetime.now().isoformat(), "log_directory": log_dir},
            "progression_log": [],
        }
    ...
    def _initialize_csv(self):
        ...
        with open(self.csv_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["match_number", "cycle", "player_id", "elo_rating", "elo_change", "games_played", "avg_score", "total_score", "epsilon", "timestamp"]
            )
    ...
    def log_match_results(self, players: List["Take6Player"], cycle: int = None):
        ...
        match_data = {"match_number": self.match_counter, "cycle": self.cycle_counter, "timestamp": timestamp, "players": []}
        ...
        self.logger.info(f"Match {self.match_counter} completed. Top performers: " + ", ".join([f"{p.player_id}({p.elo_rating:.1f})" for p in top_performers]))
    ...
    def log_cycle_summary(self, players: List["Take6Player"], cycle: int):
        ...
        summary = {
            ...
            "statistics": {
                ...
                "best_player": {"id": best_player.player_id, "elo": round(best_player.elo_rating, 2), "games": best_player.games_played},
                "worst_player": {"id": worst_player.player_id, "elo": round(worst_player.elo_rating, 2), "games": worst_player.games_played},
            },
        }

        # Log cycle summary
        self.logger.info(f"Cycle {cycle} completed: avg_elo={avg_elo:.1f}, " f"spread={max_elo-min_elo:.1f}, best={best_player.player_id}({max_elo:.1f})")
